...
# ---------------------------------------
# version 2 with thingspeak data collection
...
class PlantBrain:
    ...
    def send_to_thingspeak(self, soil, light, temp):
        """Sends data to ThingSpeak every 16 seconds (Buffer for free tier limit)"""
        now = time.time()
        # Free tier requires >15s delay between updates
        if now - self.last_thingspeak_update < 16: 
            return

        try:
            payload = {
                "api_key": THINGSPEAK_API_KEY,
                "field1": round(soil, 1), # Soil
                "field2": round(light, 1), # Light
                "field3": round(temp, 1)  # Temp
            }
            # Sending request with a short timeout to prevent blocking the code
            requests.get(THINGSPEAK_URL, params=payload, timeout=2)
            print(f"🚀 ThingSpeak Updated: Soil={round(soil,1)}, Light={round(light,1)}, Temp={round(temp,1)}")
            self.last_thingspeak_update = now
...
async def sensor_data(sid, data):
    ...
    # --- STEP 4: CLOUD SYNC (THINGSPEAK) ---
    # Trigger the ThingSpeak update logic (checks internal timer automatically)
    plant.send_to_thingspeak(avg_soil, avg_light, avg_temp)
